      ...
      <section className="token">
        <div className="wrap">
          <div className="p-1">
            <h1>No $RealT?</h1>
            <p>
              To enter the lottery, you need to buy $RealT properties first.
            </p>
            <div>
              <h2>BUY $REALT on:</h2>
        ...
        <div className="wrap">
          <div className="p-1">
            <h1>Stake $RealT</h1>
            <p>
              Once you bought properties' tokens you can stake them to get
              tickets. You can stake them for as long as you want and you can
              unstake them whenever you want by using the tickets.
            </p>
            <div>
              <button
                className="btn-blue"
                onClick={() => {
                  ...
                  setShowEnterLotteryModal(false);
                }}
              >
                STAKE $REALT
              </button>
          ...
          {ShowStakeModal && (
            <Modal
              title="Stake $RealT"
              actionTxt="STAKE"
              description={"Select the $RealT you want to stake :"}
          ...
          {ShowUnstakeModal && (
            <Modal
              title="Unstake $RealT"
              actionTxt="UNSTAKE"
              close={() => setShowUnstakeModal(false)}
              list={[]}
              onclick={(elem) => alert("unstake")}
            />
          )}
        </div>
        <div className="wrap">
          <div className="p-1">
            ...
            <p>
              ...
              You can enter the lottery with your tickets after having staked
              the underlying property for one week (to avoid abuse).
            </p>
            <button
              className="btn-blue"
              onClick={() => {
                setShowEnterLotteryModal(true);
                setShowStakeModal(false);
                setShowUnstakeModal(false);
              }}
            >
          ...
          {ShowEnterLotteryModal && (
            <Modal
              title="Enter $RealT Lottery"
              actionTxt="ENTER"
              close={() => setShowEnterLotteryModal(false)}
              list={[]}
              onclick={(elem) => alert("enter")}
            />
          )}
        </div>
      </section>
